...
import uk.gov.hmrc.exports.SplitNotificationsCollectionSpec._
import uk.gov.hmrc.exports.base.UnitSpec
import uk.gov.hmrc.exports.migrations.changelogs.notification.SplitNotificationsCollection
  ...
  "SplitNotificationsCollection migration definition" should {
    ...
    "not make any changes" when {
      ...
      "there are multiple ParsedNotifications and complete set of corresponding UnparsedNotifications" in {
        ...
        val allTestNotifications = allParsedNotifications zip allUnparsedNotifications

        allTestNotifications.foreach {
        ...
        allTestNotifications.foreach {
          case (parsed, unparsed) =>
            val parsedNotificationAfter = parsedNotificationsAfter.find(_.getString("_id") == parsed.getString("_id")).get
            val unparsedNotificationAfter = unparsedNotificationsAfter.find(_.getString("_id") == unparsed.getString("_id")).get

            compareJson(parsedNotificationAfter.toJson, parsed.toJson)
            compareJson(unparsedNotificationAfter.toJson, unparsed.toJson)
            unparsedNotificationAfter.get("item", classOf[Document]).getString("id") mustBe parsedNotificationAfter.getString(
    ...
    "add missing UnparsedNotifications and update ParsedNotifications' unparsedNotificationId fields" when {
      ...
      "there is single ParsedNotification" in {
        ...
        unparsedNotificationAfter.get("item", classOf[Document]).getString("id") mustBe parsedNotificationAfter.getString("unparsedNotificationId")
        unparsedNotificationAfter.get("item", classOf[Document]).getString("actionId") mustBe parsedNotificationAfter.getString("actionId")
        unparsedNotificationAfter.get("item", classOf[Document]).getString("payload") mustBe parsedNotification_1.getString("payload")
        unparsedNotificationAfter.getString("status") mustBe "succeeded"
      }

      "there are multiple ParsedNotifications coming as an outcome from a single payload" in {
        val testData = TestData_1
        val parsedNotification_1 = Document.parse(testData.parsedNotificationBeforeChanges)
        val parsedNotification_2 = Document.parse(testData.parsedNotificationBeforeChanges)
        val parsedNotification_3 = Document.parse(testData.parsedNotificationBeforeChanges)
        parsedNotification_1.put("_id", "5fc0d62750c11c0797a24571")
        parsedNotification_2.put("_id", "5fc0d62750c11c0797a24572")
        parsedNotification_3.put("_id", "5fc0d62750c11c0797a24573")
        ...
        allParsedNotificationsBeforeChanges.foreach { parsedNotificationBefore =>
          parsedNotificationsAfter.exists(_.getString("_id") == parsedNotificationBefore.getString("_id"))
        }

        parsedNotificationsAfter.foreach { parsedNotificationAfter =>
          val unparsedNotificationAfter = unparsedNotificationsAfter.head
          val expectedParsedNotificationAfterChanges = Document.parse(testData.parsedNotificationAfterChanges)

          expectedParsedNotificationAfterChanges.put("_id", parsedNotificationAfter.getString("_id"))
          compareJson(parsedNotificationAfter.toJson, expectedParsedNotificationAfterChanges.toJson)

          unparsedNotificationAfter.get("item", classOf[Document]).getString("id") mustBe parsedNotificationAfter.getString("unparsedNotificationId")
          unparsedNotificationAfter.get("item", classOf[Document]).getString("actionId") mustBe parsedNotificationAfter.getString("actionId")
          unparsedNotificationAfter.get("item", classOf[Document]).getString("payload") mustBe parsedNotificationAfter.getString("payload")
          unparsedNotificationAfter.getString("status") mustBe "succeeded"
        }
      }
      ...
      "there are multiple ParsedNotifications coming as an outcome from different payloads" when {
        ...
        "actionIds are the same" in {
          ...
          parsedNotificationsAfter.foreach { parsedNotificationAfter =>
            val unparsedNotificationAfter = unparsedNotificationsAfter
              .find(_.get("item", classOf[Document]).getString("id") == parsedNotificationAfter.getString("unparsedNotificationId"))
              .get
      ...
      "there are multiple ParsedNotifications and a partial set of corresponding UnparsedNotifications" in {
        ...
        allParsedNotificationsBeforeChanges.foreach { parsedNotificationBefore =>
          ...
          unparsedAfterChanges.get.get("item", classOf[Document]).getString("actionId") mustBe parsedAfterChanges.get.getString("actionId")
          unparsedAfterChanges.get.getString("status") mustBe "succeeded"

          if (parsedNotificationBefore.containsKey("payload")) {
            println(unparsedAfterChanges)
    ...
    "remove ParsedNotification" when {
      ...
      "there is unparsable Notification and corresponding UnparsedNotification" in {
        ...
        unparsedNotificationsAfter.foreach { unparsedNotificationAfter =>
          unparsedNotificationAfter.get("item", classOf[Document]).getString("actionId") mustBe unparsedNotification.getString("actionId")
          unparsedNotificationAfter.get("item", classOf[Document]).getString("payload") mustBe unparsedNotification.getString("payload")
        }
      }
    }
  }
